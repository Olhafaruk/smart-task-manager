...
async def test_update_task(test_client):
    create = await test_client.post(
        "/tasks/", json={"title": "Old title", "description": "Old desc"}
    )
...
async def test_delete_task(test_client):
    create = await test_client.post("/tasks/", json={"title": "To delete"})
    task_id = create.json()["id"]
...
async def test_update_foreign_task(test_client):
    create = await test_client.post("/tasks/", json={"title": "My task"})
    task_id = create.json()["id"]
